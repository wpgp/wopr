...
downloadData <- function(dat, outdir='wopr_downloads', maxsize=100, dialogue=T){
  for(i in 1:nrow(dat)){
    ...
    if(!file.exists(filepath) | !dat[i,'hash']==md5sum(filepath)){
      ...
      if(fsize > maxsize){
        ...
        # exit with warning
        warning(fname,' was not downloaded because it requires ',fsize,' MB of disk space which exceeds maxsize (',maxsize,' MB).',
                call.=F)
