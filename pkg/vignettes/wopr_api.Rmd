...
The WOPR REST API provides a way for web servers and other computers to communicate with the [WorldPop Open Population Repository](https://wopr.worldpop.org) to submit requests for data and to retrieve results. This can be used to:
...
**Retrieve Results**  
https://api.worldpop.org/v1/tasks/[task_id]  
...
**country** A three letter code to identify the country that the dataset represents. WOPR uses [ISO country codes](https://en.wikipedia.org/wiki/ISO_3166-1_alpha-3) to abbreviate country names. For example `'NGA'` regers to Nigeria.  
...
**doi** is the official published DOI (digital object identifier) that can be used for official citations. The DOI refers to a stable and permanent version of the file that is stored at [ftp://ftp.worldpop.org/repo/wopr](ftp://ftp.worldpop.org/repo/wopr). These files may differ slightly from the files stored at [wopr.worldpop.org](https://wopr.worldpop.org) if minor changes were made since the time of the DOI publication.  
...
**iso3** The [ISO country code](https://en.wikipedia.org/wiki/ISO_3166-1_alpha-3) of the country to query (e.g. `NGA`).  
...
An example of querying the result:   https://api.worldpop.org/v1/tasks/2a6a2883-3fd7-5fbf-832c-86e5f35e7c5e  
...
This request will return the population of children under five at the specified point location. The task id was `38f18d6e-d7d8-5886-828a-45c29da7f766`. This result can be retrieved here:  
...
An example of querying the result:   https://api.worldpop.org/v1/tasks/06b50b9f-d94d-50b9-916a-1d34b767d00a  
...
An example of querying the result:   https://api.worldpop.org/v1/tasks/b6418707-d795-56ef-9d06-c469e3697782  
